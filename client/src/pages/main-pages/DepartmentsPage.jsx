            ...
            <div className="flex-grow">
                ...
                {departments.map((department) => (
                    <div key={department._id} className="p-4 mt-6">
                        <Link to={`/edit-department/${department._id.toString()}`} className="rounded-md border w-fit p-2 text-lg font-semibold">
                            {department.name}
                        </Link>
...
//         `/companies/company/departments/${companyId}`);
//     console.log([departmentsData])
//     setDepartments([departmentsData]);
//     const employeesData = await fetchData("/employees/employees");
//     const shiftsData = await fetchData("/shifts/shifts");

//     const updatedEmployees = employeesData.map(employee => {
//         const shiftsForEmployee = shiftsData.filter(shift => shift.employees?.some(emp => emp._id === employee._id));
//         return { ...employee, shifts: shiftsForEmployee };
//     });
//     setEmployeesWithShifts(updatedEmployees);
// }
